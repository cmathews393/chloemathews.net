...
async function main() {
  ...
  try {
    ...
    await page.addStyleTag({
      content: `
      @media print {
        /* Hide the site navbar */
        header:has(> a[href="/nogimmick"]), header[role="banner"] { display: none !important; }

        /* Force all resume text to dark so it shows on a white print background */
        #resume-content, #resume-content * {
          color: #111 !important;
        }
        /* Keep accent name color readable */
        #resume-content h1 {
          color: #c0006a !important;
        }
        /* Ensure pill/link elements are visible */
        #resume-content a {
          color: #333 !important;
        }
        /* Solid white background so nothing bleeds through */
        #resume-content {
          background: #fff !important;
          border: none !important;
          box-shadow: none !important;
        }
        #resume-content aside, #resume-content main {
          background: #fff !important;
        }
      }
    `,
    ...
    await page.evaluate(() => {
      const setImportant = (el, prop, value) =>
        el && el.style.setProperty(prop, value, "important");
